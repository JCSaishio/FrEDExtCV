...
class Spooler:
    ...
    def stop_motor(self) -> None:
        """Stop driving the spooling motor (zero duty) and clear PID state."""
        try:
            if getattr(self, "motor_pwm", None) is not None:
            ...
            print(f"Error stopping spooling motor: {e}")

    def monitor_rpm(self, current_time: float) -> None:
        """Read and graph the spooler RPM WITHOUT driving the motor.
        ...
        try:
            ...
            # Guarantee no motor output while monitoring.
            if getattr(self, "motor_pwm", None) is not None:
                self.motor_pwm.ChangeDutyCycle(0)
            if self.pwm is not None:
                self.pwm.ChangeDutyCycle(0)
            ...
            print(f"Error in spooler monitor: {e}")
    ...
    def dc_motor_close_loop_control(self, current_time: float) -> None:
        ...
        try:
            ...
            setpoint_rpm = self.gui.get_motor_setpoint()
            motor_kp, motor_ki, motor_kd = self.gui.get_motor_pid()
